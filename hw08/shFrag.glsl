...
void main() {
    ...
    float dotNormLight = float(int(dot(norm, lightDir) * toonLevel))/toonLevel + 0.5/toonLevel;
    ...
    float spec = 0.0;
    if (dotNormLight > 0.0) {
        spec = float(int(pow(max(dot(viewDir, reflectDir), 0.0), material.shininess)*toonLevel))/toonLevel;
