...
function setupMouseEvents() {
    function handleMouseDown(event) {
        ...
        if(!isCircleDrawing && !circleExist){
            let [glX, glY] = convertToWebGLCoordinates(x, y);
            center = [glX, glY];
            isCircleDrawing = true;
        }

        if(circleExist && !isDrawing && !startPoint) {
            let [glX, glY] = convertToWebGLCoordinates(x, y);
            startPoint = [glX, glY];
            isDrawing = true; 
        }
    ...
    function handleMouseMove(event) {
        if(isCircleDrawing){
            ...
            let [glX, glY] = convertToWebGLCoordinates(x, y);
            r = Math.sqrt((center[0]-glX)**2+(center[1]-glY)**2);
            render();
        }
        if(isDrawing){
            const rect = canvas.getBoundingClientRect();
    ...
    function handleMouseUp() {
        if (isCircleDrawing && r){
            updateText(textOverlay, "Circle: center ("+center[0].toFixed(2)+", "+center[1].toFixed(2)+") radius = "+r.toFixed(2));
        ...
        if(isDrawing && tempEndPoint) {
            ...
            isDrawing = false;
            interPoints = intersection(center,r, startPoint, tempEndPoint);
            if(interPoints.length==4){
                updateText(textOverlay3, "Intersection Points: 2 Point 1: ("+interPoints[0].toFixed(2)+", "+interPoints[1].toFixed(2)+") Point 2: (" +interPoints[2].toFixed(2)+", "+interPoints[3].toFixed(2) +")");
            }
            else if(interPoints.length == 2){
                updateText(textOverlay3, "Intersection Points: 1 Point 1: ("+interPoints[0].toFixed(2)+", "+interPoints[1].toFixed(2)+")");
            }
            else{
...
function circle(c, r, points = 360) { //중심 좌표, 반지름
    let vertices = [];
    let degree = 0;
    for(var i = 0; i <= points; i++){
        degree =  i * Math.PI/points*2
        let x = c[0]+Math.cos(degree)*r
        let y = c[1]+Math.sin(degree)*r
        const vertex = [c[0]+Math.cos(degree)*r, c[1]+Math.sin(degree)*r];
        vertices.push(...vertex);
    }
    return vertices;
}

function intersection(center,r, startPoint, tempEndPoint){
    let a = tempEndPoint[0] - startPoint[0];
    let b = startPoint[0];
    let c = tempEndPoint[1] - startPoint[1];
    let d = startPoint[1];
    let e = center[0];
    let f = center[1];
    ...
    if(Math.abs(D) < 0.01){
        ...
        if(Math.abs(t1-0.5) <= 0.5){
            result.push(a*t1+b, c*t1+d);
        }
        if(Math.abs(t2-0.5) <= 0.5){
            result.push(a*t2+b, c*t2+d);
        }
    }
    return result;
}
...
function render() {
    ...
    if(interPoints.length != 0){
        ...
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(interPoints),gl.STATIC_DRAW);
        gl.bindVertexArray(vao);
        gl.drawArrays(gl.POINTS, 0, interPoints.length/2);
    }
